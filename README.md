...
Важно про кластеризацию в текущей реализации:
- этап детекта новизны управляется `method` и может быть только `kmeans_distance` или `lof`;
- этап группировки **уже найденных** новых объектов делается через `AgglomerativeClustering` с авто-числом кластеров (`2..10`, пропорционально количеству novel-объектов), это не вынесено в YAML как отдельный переключатель;
- явный выбор метрики расстояния для кластеризации новых тем пока не параметризован в конфиге (используется поведение по умолчанию из `sklearn`).
...
### 9.6.1 Как менять "модель кластеризации" в текущем проекте

В проекте есть **два разных шага**, которые часто путают:

1. **Модель детекта новизны** (`analysis.novelty.method`):
   - `kmeans_distance` — новизна = расстояние до ближайшего центроида KMeans, обученного на baseline.
   - `lof` — новизна = outlier score от LocalOutlierFactor, обученного на baseline.

2. **Кластеризация найденных novel-объектов**:
   - всегда `AgglomerativeClustering` (иерархическая кластеризация) в `src/complaints_trends/novelty.py`;
   - число кластеров выбирается автоматически: `n_clusters = min(max(2, len(novel)//10), 10)`.

То есть в YAML вы сейчас переключаете только **детектор** (`kmeans_distance`/`lof`), а не алгоритм финальной группировки новых тем.

### 9.6.2 Куда приводит каждая настройка

#### `analysis.novelty.method`
- `kmeans_distance`:
  - лучше, когда в baseline есть несколько устойчивых "областей нормы";
  - проще интерпретировать (дистанция до нормы).
- `lof`:
  - лучше ловит локальные аномалии в сложной геометрии;
  - чувствительнее к шуму и масштабу данных.

#### `analysis.novelty.svd_components`
- Меньше (например 50): быстрее, грубее, может пропускать узкие темы.
- Больше (например 150–300): тоньше семантика, но риск шума и роста времени.

#### `analysis.novelty.kmeans_k`
(влияет только при `method: kmeans_distance`)
- Ниже (10–20): крупная "норма", выше риск пропустить небольшие, но валидные отклонения.
- Выше (50–100): норма более детализирована, выше риск считать шум нормой (или наоборот нестабильность).

#### `analysis.novelty.threshold_percentile`
- 99: очень строгий порог, мало novel, выше precision, ниже recall.
- 95: мягкий порог, много novel, выше recall, но больше ложных срабатываний.

#### `analysis.novelty.min_cluster_size`
- Фильтр на отчётные кластеры новых тем (после кластеризации).
- Ниже (5–10): увидите больше мелких сигналов.
- Выше (20+): отчёты чище, но редкие важные кейсы могут исчезнуть.

### 9.6.3 Про дистанции и метрики: что доступно сейчас

В текущем коде явный выбор distance metric в конфиге **не реализован**:
- для `kmeans_distance` используется евклидово расстояние KMeans (`km.transform(...)`);
- для `lof` используется внутренняя метрика `LocalOutlierFactor` по умолчанию (minkowski, `p=2`);
- для `AgglomerativeClustering` отдельная метрика также не задаётся (поведение `sklearn` по умолчанию для используемой версии).

Если нужен явный контроль (`cosine`, `manhattan`, `precomputed`) — это доработка `src/complaints_trends/novelty.py` + расширение `configs/project.yaml`.

### 9.6.4 Практические шаблоны конфигураций (все рабочие варианты в текущем проекте)

Ниже приведены **все реально поддерживаемые сейчас** варианты модели новизны.

#### Вариант A — базовый стабильный (`kmeans_distance`)
...
Зачем: универсальный старт для production-мониторинга, когда нужен баланс сигнал/шум.

Что делает: строит "облака нормы" по baseline через KMeans и помечает как novel точки далеко от центроидов.

Когда применять: первый запуск, регулярный monthly compare, когда нет уверенности в структуре данных.

---

#### Вариант B — чувствительный поиск новых тем (`kmeans_distance`, мягкий порог)
...
Зачем: максимально не пропускать потенциально новые темы (высокий recall).

Что делает: снижает порог новизны и детализирует норму. Получите больше кандидатов и больше ручной проверки.

Когда применять: этап discovery, запуск нового продукта/канала, ожидание всплеска новых сценариев.

---

#### Вариант C — строгий алертинг (`kmeans_distance`, жёсткий порог)
...
Зачем: уменьшить ложные тревоги и оставить только самые аномальные кейсы.

Что делает: поднимает порог и укрупняет норму. Novel-тем будет мало, но они обычно ярко выражены.

Когда применять: зрелый мониторинг с дорогой ручной валидацией, ограниченный ресурс аналитиков.

---

#### Вариант D — локальные аномалии (`lof`)
...
analysis:
  novelty:
    ...
    method: "lof"
    svd_components: 100
    kmeans_k: 40  # не используется в lof, можно оставить как есть
    threshold_percentile: 97
    min_cluster_size: 15
```

Зачем: искать нестандартные группы, которые плохо описываются расстоянием до центроидов.

Что делает: обучает LOF на baseline и оценивает "насколько локально выбивается" каждая новая запись.

Когда применять: сложная структура текста, где одна глобальная "норма" плохо определена.

### 9.6.5 Как применять на практике (пошагово)

1. Выберите шаблон (A/B/C/D) и внесите его в `configs/project.yaml`.
...
3. Откройте отчёт `reports/compare_report.html` и проверьте:
   - число novel-записей,
   - размер кластеров,
   - осмысленность примеров.
4. Если novel слишком много — поднимайте `threshold_percentile` или `min_cluster_size`.
5. Если novel слишком мало — снижайте `threshold_percentile` и/или увеличивайте `svd_components`.

### 9.6.6 Матрица "цель -> рекомендованный пресет"

- Цель: **не пропустить новые проблемы** -> Вариант B.
- Цель: **минимум ложных срабатываний** -> Вариант C.
- Цель: **баланс и простая интерпретация** -> Вариант A.
- Цель: **локальные аномалии в сложной структуре** -> Вариант D.
