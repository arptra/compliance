...
class OverviewService:
    ...
    def get_overview(self, params: dict) -> OverviewResponse:
        ...
        ds = df.copy()
        ds["date"] = pd.to_datetime(ds[date_col], errors="coerce").dt.date
        ...
        if not hist.empty:
            ...
            if hist_col:
                hist["date"] = pd.to_datetime(hist[hist_col], errors="coerce").dt.normalize()
                hist = hist.groupby("date").size().rename("actual").reset_index()

        ts["expected"] = compute_expected_series(hist if not hist.empty else pd.DataFrame(), ts, baseline_start, baseline_end, params.get("baseline_mode", "previous_period"))
        ts["delta"] = ts["actual"] - ts["expected"].fillna(0)

        baseline_df = self.loader.load_prepare()
        baseline_df = filter_by_date(baseline_df, "event_time" if "event_time" in baseline_df.columns else "date", str(baseline_start.date()) if baseline_start is not None else None, str(baseline_end.date()) if baseline_end is not None else None)
        metrics = compute_category_compare_metrics(df, baseline_df if not baseline_df.empty else pd.DataFrame(columns=["category"]))

        total = float(len(df))
        expected_total = float(ts["expected"].sum()) if ts["expected"].notna().any() else 0.0
        kpis = [
        ...
        alerts = [CategoryMetric(**row, anomaly_score=max((row.get("delta_pct") or 0), 0.0)) for row in metrics.sort_values("delta_pct", ascending=False).head(10).to_dict(orient="records")]
        tvs = [TimeSeriesPoint(date=r["date"].date(), actual=float(r["actual"]), expected=(None if pd.isna(r["expected"]) else float(r["expected"])), delta=float(r["delta"])) for _, r in ts.iterrows()]
        ...
        return OverviewResponse(
            ...
            sparklines={"overall": ts["actual"].astype(float).tolist()},
        )
