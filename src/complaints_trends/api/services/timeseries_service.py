...
def compute_expected_series(
    ...
    expected = baseline["actual"].mean()
    return pd.Series([float(expected)] * len(actual_series), index=actual_series.index)
...
def compute_category_compare_metrics(actual_df: pd.DataFrame, baseline_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["delta_pct"] = out.apply(
        lambda x: (x["delta_abs"] / x["baseline_count"]) if x["baseline_count"] else None,
        axis=1,
    )
