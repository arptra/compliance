...
@app.command("prepare")
def prepare_cmd(config: str, pilot: bool = False, month: str | None = None, limit: int | None = None, mock_llm: bool = False):
...
def infer_month_cmd(config: str, excel: str, month: str):
    cfg = load_config(config)
...
@app.command("compare")
def compare_cmd(config: str, new_month: str, baseline_range: str):
    cfg = load_config(config)
...
@app.command("demo")
def demo_cmd(config: str = "configs/project.yaml"):
    cfg = load_config(config)
