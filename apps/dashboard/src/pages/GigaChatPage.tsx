      ...
      <div className='async-workers-grid'>
        ...
        <label className='lab-field async-workers-slider'>
          ...
          <input
            ...
            onChange={(event) => setAsyncWorkerCount(clampAsyncWorkerCount(event.target.value))}
          />
          <small className='lab-field-help'>При HTTP 429 backend ставит запросы в общую очередь. После первого успешного контрольного запроса очередь снова продолжает работу параллельно.</small>
        </label>
        <div className='async-workers-summary'>
          <strong>{asyncWorkerCount === 1 ? 'Последовательно' : 'Параллельно'}</strong>
          <span>{asyncWorkerCount === 1 ? 'Следующий запрос стартует после завершения предыдущего.' : `До ${asyncWorkerCount} строк могут быть в работе одновременно. Новый HTTP 429 снова временно включает последовательную очередь.`}</span>
        </div>
      </div>
    </section>
