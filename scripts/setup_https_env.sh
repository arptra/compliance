...
setup_https_env() {
  ...
  if [[ ! -r "${cert_file}" || ! -r "${key_file}" ]]; then
    ...
    echo "Set these two paths in ${app_root}/scripts/start_vm_https.sh." >&2
    return 1
  fi
  ...
  "${python_bin}" "${app_root}/scripts/prepare_https.py" \
    --cert "${cert_file}" --key "${key_file}" --output "${fullchain_file}" || return 1
