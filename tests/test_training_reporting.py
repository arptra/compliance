...
from complaints_trends.train_models import _build_time_scatter_frame, _coerce_binary_labels, _normalize_subcategory_by_taxonomy, train
...
def test_train_generates_human_report_and_charts(tmp_path):
    ...
    cfg.training.validation.val_from = "2025-01-03 00:00:00"
    cfg.training.validation.val_to = "2025-01-04 23:59:59"
    cfg.training.validation.split_mode = "time"
    cfg.training.validation.val_from = "2025-01-03 00:00:00"
    cfg.training.validation.val_to = "2025-01-04 23:59:59"
    cfg.training.validation.split_mode = "time"
...
def test_coerce_binary_labels_handles_string_values():
    values = ["true", "false", "1", "0", "yes", "no", "жалоба", "не жалоба", "", None]
    out = _coerce_binary_labels(values)
    assert out.tolist() == [True, False, True, False, True, False, True, False, False, False]


def test_train_does_not_treat_string_false_as_complaint(monkeypatch, tmp_path):
    cfg = load_config("configs/project.yaml").model_copy(deep=True)
    cfg.prepare.output_parquet = str(tmp_path / "prepared.parquet")
    ...
    df = pd.DataFrame(
        [
            {"row_id": "r1", "event_time": "2025-01-01", "client_first_message": "не работает", "is_complaint_llm": True, "complaint_category_llm": "TECHNICAL", "complaint_subcategory_llm": "login_issue"},
            {"row_id": "r2", "event_time": "2025-01-02", "client_first_message": "спасибо", "is_complaint_llm": False, "complaint_category_llm": "OTHER", "complaint_subcategory_llm": "UNKNOWN"},
            {"row_id": "r3", "event_time": "2025-01-03", "client_first_message": "ошибка", "is_complaint_llm": True, "complaint_category_llm": "TECHNICAL", "complaint_subcategory_llm": "app_crash"},
            {"row_id": "r4", "event_time": "2025-01-04", "client_first_message": "вопрос", "is_complaint_llm": False, "complaint_category_llm": "OTHER", "complaint_subcategory_llm": "UNKNOWN"},
        ]
    )
    df["event_time"] = pd.to_datetime(df["event_time"])
    df.to_parquet(cfg.prepare.output_parquet, index=False)
    ...
    review = pd.DataFrame(
        [
            {"row_id": "r1", "is_complaint_gold": "true", "category_gold": "TECHNICAL"},
            {"row_id": "r2", "is_complaint_gold": "false", "category_gold": "OTHER"},
            {"row_id": "r3", "is_complaint_gold": "true", "category_gold": "TECHNICAL"},
            {"row_id": "r4", "is_complaint_gold": "false", "category_gold": "OTHER"},
        ]
    )
    ...
    assert captured["complaints_df_len"] == 2
