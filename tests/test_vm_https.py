...
class HTTPSCertificateTests(unittest.TestCase):
    ...
    def test_vm_entrypoints_validate_before_launch_or_stop(self):
        with tempfile.TemporaryDirectory(prefix="vm with spaces-") as temporary:
            ...
            # Edit the variables inside the launcher once, then use fresh shells.
            ...
            result = subprocess.run(
                ...
                env=clean_env(PUBLIC_HOST="override.example.com", API_PORT="18443", HTTPS_ENABLED="0", PUBLIC_SCHEME="http"),
                capture_output=True, text=True,
            )
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn("DISPATCH:1:https", result.stdout)
            self.assertIn("https://override.example.com:18443", result.stdout)
            # A broken certificate path in the script must not stop a running stack.
            launcher.write_text(launcher.read_text().replace(":-saved certs/privkey.pem}", ":-missing.key}"))
