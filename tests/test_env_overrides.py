...
def test_llm_config_without_key_file_password_env_is_valid(monkeypatch, tmp_path: Path):
    cfg_path = tmp_path / "cfg_no_pwd_env.yaml"
    cfg = {
        "input": {
            "input_dir": "data/raw",
            "file_glob": "*.xlsx",
        ...
        "prepare": {"pilot_limit": 100, "date_from": None, "date_to": None, "output_parquet": "a.parquet", "pilot_parquet": "b.parquet", "pilot_review_xlsx": "c.xlsx"},
        "training": {
            "text_field": "client_first_message",
            "complaint_threshold": 0.5,
            "vectorizer": {"word_ngram": [1, 2], "char_ngram": [3, 5], "max_features_word": 1000, "max_features_char": 1000, "min_df": 1, "max_df": 1.0},
            "classifier": {"complaint": "logreg", "category": "linearsvc"},
            "validation": {"split_mode": "time", "val_from": None, "val_to": None},
            "model_dir": "models",
        },
        "analysis": {"novelty": {"enabled": True, "method": "kmeans_distance", "svd_components": 10, "kmeans_k": 3, "threshold_percentile": 95, "min_cluster_size": 2}, "reports_dir": "reports"},
        "files": {"deny_tokens_path": "configs/deny_tokens.txt", "extra_stopwords_path": "configs/extra_stopwords.txt", "categories_seed_path": "configs/categories_seed.yaml"},
    ...
    cfg_path.write_text(yaml.safe_dump(cfg, allow_unicode=True), encoding="utf-8")
    loaded = load_config(cfg_path)
    assert loaded.llm.key_file_password_env is None


def test_unknown_config_fields_are_ignored_and_do_not_override_known_values(tmp_path: Path):
    cfg_path = tmp_path / "cfg_extra_fields.yaml"
    cfg = {
        "input": {
            "input_dir": "data/raw",
            "file_glob": "*.xlsx",
            ...
            "dialog_column": "dialog_text",
            "encoding": "utf-8",
            "unknown_input_flag": "SHOULD_BE_IGNORED",
        },
        "client_first_extraction": {
            "enabled": True,
            "client_markers": ["CLIENT"],
        ...
        "llm": {
            ...
            "cache_db": "data/interim/cache.sqlite",
            "prompt_version": "v1",
            "unknown_llm_field": "SHOULD_BE_IGNORED",
        },
        "prepare": {
            "pilot_limit": 100,
            "date_from": None,
            "date_to": None,
            "output_parquet": "a.parquet",
            "pilot_parquet": "b.parquet",
            "pilot_review_xlsx": "c.xlsx",
            "unused_prepare_toggle": 999,
        },
        "training": {
            "text_field": "client_first_message",
            "complaint_threshold": 0.5,
            "vectorizer": {
                ...
                "min_df": 1,
                "max_df": 1.0,
                "vectorizer_extra": "ignored",
            },
            "classifier": {"complaint": "logreg", "category": "linearsvc"},
            "validation": {"split_mode": "time", "val_from": None, "val_to": None, "val_month": "ignored"},
            "model_dir": "models",
            "random_new_field": "ignored",
        },
        "analysis": {
            "novelty": {
                "enabled": True,
    ...
    loaded = load_config(cfg_path)
    assert loaded.input.input_dir == "data/raw"
    assert loaded.llm.base_url == "https://from-config"
    assert loaded.prepare.pilot_limit == 100
    assert loaded.training.text_field == "client_first_message"
    assert loaded.training.validation.split_mode == "time"
