...
def _setup(tmp_path: Path) -> Path:
    ...
    pd.DataFrame({
        'event_time': ['2025-01-01', '2025-01-01', '2025-01-02', '2024-12-30', '2024-12-31'],
        'category': ['A', 'B', 'A', 'A', 'C'],
        'text': ['t1', 't2', 't3', 't4', 't5'],
...
def test_executive_report_payload_schema(tmp_path: Path):
    client = TestClient(create_app(str(_setup(tmp_path))))
    r = client.post('/api/reports/executive', json={'date_from': '2025-01-01', 'date_to': '2025-01-02', 'compare_mode': 'previous_period'})
    assert r.status_code == 200
    payload = r.json()
    assert 'kpis' in payload
    assert 'charts' in payload
    assert 'summary' in payload
    assert payload['definitions'].get('baseline')
